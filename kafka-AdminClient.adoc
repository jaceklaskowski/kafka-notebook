...
[TIP]
====
Enable `ALL` logging level for `kafka.admin.AdminClient` logger to see what happens inside.

Add the following line to `config/tools-log4j.properties`:

```
log4j.logger.kafka.admin.AdminClient=ALL
```
...
`send` requests <<client, ConsumerNetworkClient>> to link:kafka-consumer-internals-ConsumerNetworkClient.adoc#send[send] the input `request` to the `target`.
...
`findAllBrokers` returns the nodes from the link:kafka-common-requests-MetadataResponse.adoc#cluster[cluster metadata] of the `MetadataResponse`.
