...
`handleGroupImmigration` simply requests the <<groupManager, GroupMetadataManager>> to <<kafka-GroupMetadataManager.adoc#scheduleLoadGroupAndOffsets, scheduleLoadGroupAndOffsets>> (for the given offset and the <<onGroupLoaded, onGroupLoaded>> callback)

NOTE: `handleGroupImmigration` is used when...FIXME
...
`handleGroupEmigration` simply requests the <<groupManager, GroupMetadataManager>> to <<kafka-GroupMetadataManager.adoc#removeGroupsForPartition, removeGroupsForPartition>> (for the given offset and the <<onGroupUnloaded, onGroupUnloaded>> callback)

NOTE: `handleGroupEmigration` is used when...FIXME
