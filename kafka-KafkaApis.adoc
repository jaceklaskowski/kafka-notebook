...
* [[controller]] <<kafka-KafkaController.adoc#, KafkaController>>
...
NOTE: `handleStopReplicaRequest` is used exclusively to when `KafkaApis` is requested to handle a <<StopReplica, StopReplica>> request.
...
NOTE: `handleUpdateMetadataRequest` is used exclusively to when `KafkaApis` is requested to handle a <<UpdateMetadata, UpdateMetadata>> request.
...
NOTE: `handleOffsetCommitRequest` is used exclusively to when `KafkaApis` is requested to handle a <<OffsetCommit, OffsetCommit>> request.
...
NOTE: `handleDescribeGroupRequest` is used exclusively to when `KafkaApis` is requested to handle a <<DescribeGroups, DescribeGroups>> request.
