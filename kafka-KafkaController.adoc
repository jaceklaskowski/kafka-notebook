...
`KafkaController` is a Kafka service that is part of every Kafka broker, but only one `KafkaController` is <<isActive, active>> among all Kafka brokers in a Kafka cluster.
...
`elect` requests the <<zkClient, KafkaZkClient>> for the <<kafka-zk-KafkaZkClient.adoc#getControllerId, active controller ID>> (or defaults to `-1`).

`elect`...FIXME

`elect` quits if there is an active controller ID available with the following DEBUG message to the logs:

```
Broker [activeControllerId] has been elected as the controller, so stopping the election process.
```

NOTE: `elect` is used when `ControllerEventThread` is requested to <<kafka-ControllerEventThread.adoc#doWork, process controller events>> (and processes <<kafka-ControllerEvent.adoc#Startup, Startup>> and <<kafka-ControllerEvent.adoc#Reelect, Reelect>> events).
