...
a| link:kafka-ReplicaManager.adoc[ReplicaManager] exclusively to create:
...
* link:kafka-ReplicaManager.adoc#creating-instance[Created] (and link:kafka-ReplicaManager.adoc#startup[started] immmediately) when `KafkaServer` <<startup, starts up>>

* link:kafka-ReplicaManager.adoc#shutdown[Shut down] when `KafkaServer` <<shutdown, shuts down>>
...
`createReplicaManager` simply link:kafka-ReplicaManager.adoc#creating-instance[creates] a `ReplicaManager`.

NOTE: `createReplicaManager` is used exclusively when `KafkaServer` <<startup, starts up>>.
