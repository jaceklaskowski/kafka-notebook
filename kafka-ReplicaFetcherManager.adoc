...
`ReplicaFetcherManager` is a <<kafka-AbstractFetcherManager.adoc#, AbstractFetcherManager>> of <<kafka-server-ReplicaFetcherThread.adoc#, ReplicaFetcherThreads>> that...FIXME (describe properties)
...
NOTE: `createFetcherThread` is used exclusively when `AbstractFetcherManager` is requested to  link:kafka-AbstractFetcherManager.adoc#addFetcherForPartitions[addFetcherForPartitions]
