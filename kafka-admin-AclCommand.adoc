...
`AclCommand` uses `AdminClientService` (when executed with <<bootstrap-server, --bootstrap-server>> option) or `AuthorizerService`.

`kafka-acls.sh` requires <<kafka-security-Authorizer.adoc#, Authorizer>> to be configured on a broker (when executed with <<bootstrap-server, --bootstrap-server>> option) or throws a `SecurityDisabledException`:
