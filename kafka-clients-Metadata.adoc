...
`update` notifies <<listeners, listeners>> that the link:kafka-clients-Metadata-Listener.adoc#onMetadataUpdate[metadata has been updated].
...
* `KafkaConsumer` is link:kafka-clients-consumer-KafkaConsumer.adoc#creating-instance[created] (and updates the cluster with a "bootstrap" cluster with bootstrap servers)
