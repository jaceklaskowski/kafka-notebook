...
`Metadata` describes a Kafka <<cluster, cluster>> (and is <<creating-instance, created>>) for <<kafka-clients-consumer-KafkaConsumer.adoc#metadata, KafkaConsumer>> and <<kafka-producer-KafkaProducer.adoc#metadata, KafkaProducer>>.
...
NOTE: <<kafka-clients-consumer-internals-ConsumerCoordinator.adoc#, ConsumerCoordinator>> is the only entity to register a metadata update listener.
...
* `ConsumerCoordinator` link:kafka-clients-consumer-internals-ConsumerCoordinator.adoc#poll[polls for coordinator events]

* `ConsumerNetworkClient` link:kafka-clients-consumer-internals-ConsumerNetworkClient.adoc#ensureFreshMetadata[makes sure that the metadata is fresh]
====
...
* `ConsumerNetworkClient` link:kafka-clients-consumer-internals-ConsumerNetworkClient.adoc#ensureFreshMetadata[ensureFreshMetadata]
...
* `KafkaConsumer` is requested to <<kafka-clients-consumer-KafkaConsumer.adoc#subscribe, subscribe>>

* `ConsumerCoordinator` is <<kafka-clients-consumer-internals-ConsumerCoordinator.adoc#, created>>, <<kafka-clients-consumer-internals-ConsumerCoordinator.adoc#addMetadataListener, addMetadataListener>>, <<kafka-clients-consumer-internals-ConsumerCoordinator.adoc#poll, poll>>

* `ConsumerNetworkClient` is requested to <<kafka-clients-consumer-internals-ConsumerNetworkClient.adoc#awaitMetadataUpdate, awaitMetadataUpdate>>

* `Fetcher` is requested to <<kafka-clients-consumer-internals-Fetcher.adoc#resetOffsetsAsync, resetOffsetsAsync>>, <<kafka-clients-consumer-internals-Fetcher.adoc#groupListOffsetRequests, groupListOffsetRequests>>, <<kafka-clients-consumer-internals-Fetcher.adoc#prepareFetchRequests, prepareFetchRequests>>, <<kafka-clients-consumer-internals-Fetcher.adoc#parseCompletedFetch, parseCompletedFetch>>
...
NOTE: `addListener` is used exclusively when `ConsumerCoordinator` is requested to <<kafka-clients-consumer-internals-ConsumerCoordinator.adoc#addMetadataListener, addMetadataListener>> (when <<kafka-clients-consumer-internals-ConsumerCoordinator.adoc#, created>>).
