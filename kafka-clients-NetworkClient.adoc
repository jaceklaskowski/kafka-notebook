...
`NetworkClient` is a non-blocking link:kafka-clients-KafkaClient.adoc[KafkaClient] that uses <<selector, Selectable>> for network communication (i.e. sending and receiving messages).
...
`initiateConnect` requests <<metadataUpdater, MetadataUpdater>> for link:kafka-clients-MetadataUpdater.adoc#requestUpdate[update].
...
* `DefaultMetadataUpdater` link:kafka-clients-DefaultMetadataUpdater.adoc#maybeUpdate[maybeUpdate]
====
...
NOTE: `ready` is a part of link:kafka-clients-KafkaClient.adoc#ready[KafkaClient Contract].
...
`poll` requests <<metadataUpdater, MetadataUpdater>> for link:kafka-clients-MetadataUpdater.adoc#maybeUpdate[cluster metadata update (if needed and possible)].
...
* [[metadataUpdater]] <<kafka-clients-MetadataUpdater.adoc#, MetadataUpdater>>
