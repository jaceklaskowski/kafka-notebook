...
`ConsumerConfig` is a Apache Kafka https://kafka.apache.org/21/javadoc/org/apache/kafka/common/config/AbstractConfig.html[AbstractConfig] for the <<properties, configuration properties>> of a <<kafka-clients-consumer-KafkaConsumer.adoc#, KafkaConsumer>>.
...
Default: <<kafka-clients-consumer-RangeAssignor.adoc#, RangeAssignor>>
...
* `KafkaConsumer` is <<kafka-clients-consumer-KafkaConsumer.adoc#assignors, created>>

* Kafka Streams' `StreamsConfig` is requested to `getMainConsumerConfigs`

NOTE: Required for `KafkaConsumer` to <<kafka-clients-consumer-KafkaConsumer.adoc#subscribe, subscribe to topics>>
|===
