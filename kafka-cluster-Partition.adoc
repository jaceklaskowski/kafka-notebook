...
`makeLeader` requests the given `LeaderAndIsrPartitionState` for the partition replicas (their broker IDs) and the ISR (as a list of broker IDs) and <<updateAssignmentAndIsr, updateAssignmentAndIsr>>.
...
=== [[updateAssignmentAndIsr]] Updating Replica Assignments and In-Sync Replicas -- `updateAssignmentAndIsr` Method
...
`updateAssignmentAndIsr` registers (_saves_) the partition replicas (based on the given `assignment`) in the <<remoteReplicasMap, remoteReplicasMap>> internal registry.
