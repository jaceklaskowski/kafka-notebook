...
`getOrCreateReplica` finds the <<kafka-cluster-Replica.adoc#, Replica>> in the <<allReplicasMap, allReplicasMap>> internal registry (by the given `replicaId`).

When the replica is not in the <<allReplicasMap, allReplicasMap>> internal registry, `getOrCreateReplica`...FIXME
