...
NOTE: `alterReplicaLogDirs` is used exclusively when `KafkaApis` is requested to <<kafka-KafkaApis.adoc#handleAlterReplicaLogDirsRequest, handle an AlterReplicaLogDirs request>>.
