...
`appendRecordsToLeader` basically requests the `Log` (of the leader <<kafka-cluster-Replica.adoc#, Replica>>) to <<kafka-Log.adoc#appendAsLeader, appendAsLeader>>.
...
* `ReplicaFetcherThread` is requested to <<kafka-ReplicaFetcherThread.adoc#processPartitionData, processPartitionData>>
====
