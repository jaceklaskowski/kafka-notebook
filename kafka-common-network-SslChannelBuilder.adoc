...
`sslPrincipalMapper` is a link:kafka-common-security-ssl-SslPrincipalMapper.adoc[SslPrincipalMapper] that is used to create a `SslAuthenticator` when `SslChannelBuilder` is requested to <<buildChannel, build a channel>>.
