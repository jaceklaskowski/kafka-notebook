== [[KafkaConsumer]] KafkaConsumer -- Kafka Client for Consuming Records

`KafkaConsumer` is the <<kafka-consumer-Consumer.adoc#, Kafka client to consume records from a Kafka cluster>>.

`KafkaConsumer` is the main public class that a Kafka developer use to write <<kafka-consumers.adoc#, Kafka consumers>>.
