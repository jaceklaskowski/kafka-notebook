...
* [[lookupAssignor]] Looking up the <<kafka-consumer-PartitionAssignor.adoc#, PartitionAssignor>> by name (when <<onJoinComplete, onJoinComplete>> and <<performAssignment, performAssignment>>)
...
`addMetadataListener` registers a <<kafka-clients-Metadata-Listener.adoc#, Metadata Update Listener>> (with the <<metadata, Metadata>>) that does the following:
...
* [[sessionTimeoutMs]] `sessionTimeoutMs`
* [[heartbeatIntervalMs]] `heartbeatIntervalMs`
* [[assignors]] Collection of <<kafka-consumer-PartitionAssignor.adoc#, PartitionAssignors>>
