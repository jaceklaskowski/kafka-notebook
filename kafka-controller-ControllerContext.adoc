...
NOTE: `checkValidReplicaStateChange` is used exclusively when `ZkReplicaStateMachine` is requested to <<kafka-controller-ZkReplicaStateMachine.adoc#doHandleStateChanges, handle state changes of partition replicas>>.
