...
`KafkaController` is a Kafka service that runs on every Kafka broker. Only one `KafkaController` can be <<isActive, active>> (_elected_) among all the Kafka brokers in a Kafka cluster.
