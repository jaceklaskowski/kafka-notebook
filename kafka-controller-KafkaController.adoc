...
`KafkaController` is a Kafka service that runs on every Kafka broker in a Kafka cluster, but only one `KafkaController` can be <<isActive, active>> (_elected_).
...
`KafkaController` uses the <<zkClient, KafkaZkClient>> to be notified about changes in the state of a Kafka cluster (that are reflected in changes in znodes of Apache Zookeeper).
...
| replicaStateMachine
a| [[replicaStateMachine]] <<kafka-controller-ReplicaStateMachine.adoc#, ReplicaStateMachine>>
