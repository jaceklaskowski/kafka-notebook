...
`handleCommitOffsets` requests the <<groupManager, GroupMetadataManager>> to <<kafka-GroupMetadataManager.adoc#getGroup, getGroup>> (by the given `groupId`) and then <<doCommitOffsets, doCommitOffsets>>.

If the <<groupManager, GroupMetadataManager>> could not <<kafka-GroupMetadataManager.adoc#getGroup, getGroup>>, `handleCommitOffsets`...FIXME
