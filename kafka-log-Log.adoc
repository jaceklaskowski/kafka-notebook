...
`Log` is a *partition log* that is <<apply, created>> when `LogManager` is requested to <<kafka-LogManager.adoc#loadLog, loadLog>> (when `LogManager` is <<kafka-LogManager.adoc#creating-instance, created>>) and <<kafka-LogManager.adoc#getOrCreateLog, getOrCreateLog>>.
...
NOTE: `truncateTo` is used exclusively when `LogManager` is requested to <<kafka-LogManager.adoc#truncateTo, truncateTo>>.
...
* `LogManager` is requested to <<kafka-LogManager.adoc#truncateFullyAndStartAt, truncateFullyAndStartAt>>
====
