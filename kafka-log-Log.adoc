...
`logFile` <<filenamePrefixFromOffset, creates a file name of a log segment>> (for the given `offset`, the <<LogFileSuffix, .log>> suffix and the optional `suffix`) in the given `dir` directory.

[source, scala]
----
import java.nio.file.{Files, Path}
val tmp = "/tmp/kafka-internals"
val dir = Files.createDirectory(Path.of(tmp)).toFile()
...
`offsetIndexFile`...FIXME

NOTE: `offsetIndexFile` is used when...FIXME
...
NOTE: `timeIndexFile` is used when...FIXME
...
`producerSnapshotFile`...FIXME

NOTE: `producerSnapshotFile` is used when...FIXME
...
`transactionIndexFile`...FIXME

NOTE: `transactionIndexFile` is used when...FIXME
