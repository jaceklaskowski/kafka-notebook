...
| .txnindex
a| [[TxnIndexFileSuffix]]

| .timeindex
a| [[TimeIndexFileSuffix]]
...
`hasCleanShutdownFile` returns `true` if <<CleanShutdownFile, .kafka_cleanshutdown>> file is in the parent directory of the <<dir, log directory>>. Otherwise, `hasCleanShutdownFile` is `false`.

NOTE: `hasCleanShutdownFile` is used exclusively when `Log` is <<creating-instance, created>> (to <<loadProducerState, loadProducerState>>) and requested to
...
NOTE: `isIndexFile` is used when...FIXME
...
NOTE: `isLogFile` is used when...FIXME
...
`recoverSegment`...FIXME

NOTE: `recoverSegment` is used when...FIXME
