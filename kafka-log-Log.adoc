...
`Log` is a collection of <<segments, LogSegments>> that are stored on disk in a given <<dir, log directory>> with the <<parseTopicPartitionName, name>> of the form *topic-partition* or *topic-partition.uniqueId-delete* (if marked for deletion).
