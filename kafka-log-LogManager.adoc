...
`LogManager` creates a <<kafka-log-LogCleaner.adoc#, LogCleaner>> only when the <<kafka-log-CleanerConfig.adoc#enableCleaner, enableCleaner>> flag of the given <<cleanerConfig, CleanerConfig>> is enabled (default: `true`).
