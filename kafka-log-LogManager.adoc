...
`LogManager` creates a <<kafka-log-LogCleaner.adoc#, LogCleaner>> only when the <<kafka-log-LogCleaner.adoc#enableCleaner, enableCleaner>> flag of the given <<cleanerConfig, CleanerConfig>> is enabled (default: `true`).

`LogManager` uses <<kafka-log-LogCleaner.adoc#, LogCleaner>> for...FIXME
