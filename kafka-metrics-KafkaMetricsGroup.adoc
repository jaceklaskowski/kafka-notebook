...
`KafkaMetricsGroup` is the <<contract, contract>> of <<implementations, metrics groups>> that can register *performance metrics*, e.g. <<newGauge, gauges>>, <<newHistogram, histograms>>, <<newMeter, meters>> and <<newTimer, timers>>.
