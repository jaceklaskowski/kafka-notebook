...
Kafka stores link:kafka-messages.adoc[messages] in link:kafka-topics.adoc[topics] that are link:kafka-topics.adoc#partitions[partitioned] and replicated across multiple link:kafka-broker.adoc[brokers] in a cluster. link:kafka-producers.adoc[Producers] send messages to topics from which link:kafka-consumers.adoc[consumers] read.
