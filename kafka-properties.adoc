...
a| [[replica.lag.time.max.ms]] If a follower hasn't sent any fetch requests or hasn't consumed up to the leader's log end offset (LEO) for at least this time, the leader will remove the follower from isr
