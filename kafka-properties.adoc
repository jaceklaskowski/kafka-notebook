...
When disabled, offsets have to be committed manually (synchronously using link:kafka-clients-consumer-KafkaConsumer.adoc#commitSync[KafkaConsumer.commitSync] or asynchronously link:kafka-clients-consumer-KafkaConsumer.adoc#commitAsync[KafkaConsumer.commitAsync]). On restart restore the position of a consumer using link:kafka-clients-consumer-KafkaConsumer.adoc#seek[KafkaConsumer.seek].

Used when `KafkaConsumer` is link:kafka-clients-consumer-KafkaConsumer.adoc#creating-instance[created] and creates a link:kafka-clients-consumer-internals-ConsumerCoordinator.adoc#autoCommitEnabled[ConsumerCoordinator].
...
| interceptor.classes
a| [[interceptor.classes]] Comma-separated list of link:kafka-clients-consumer-ConsumerInterceptor.adoc[ConsumerInterceptor] class names.
...
| max.poll.records
a| [[max.poll.records]] (KafkaConsumer) The maximum number of records returned from a Kafka `Consumer` when link:kafka-clients-consumer-Consumer.adoc#poll[polling topics for records].
...
Internally, `max.poll.records` is used exclusively when `KafkaConsumer` is link:kafka-clients-consumer-KafkaConsumer.adoc#creating-instance[created] (to create a link:kafka-clients-consumer-KafkaConsumer.adoc#fetcher[Fetcher]).
