...
| log.cleanup.policy
a| [[log.cleanup.policy]] Comma-separated list of cleanup (retention) policies to use on old log segments (for segments beyond the retention window)

Default: `delete`

Supported values:

* *compact* - enables log compaction on a topic
...
| log.segment.bytes
a| [[log.segment.bytes]] The maximum size of a single log segment file. Retention and cleaning are always done one file at a time so a larger segment size means fewer files but less granular control over retention.
