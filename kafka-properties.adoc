...
Default: `null` (undefined), i.e. no dedicated endpoints for controller connections

Must be defined among <<advertised.listeners, advertised.listeners>>
...
* Use link:kafka-server-KafkaConfig.adoc#getControlPlaneListenerNameAndSecurityProtocol[KafkaConfig.getControlPlaneListenerNameAndSecurityProtocol] to access the current value
...
It is an link:kafka-server-KafkaConfig.adoc#getInterBrokerListenerNameAndSecurityProtocol[error] to use together with <<inter.broker.listener.name, inter.broker.listener.name>>
