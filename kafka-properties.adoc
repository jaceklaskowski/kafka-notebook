...
a| [[authorizer.class.name]] Fully-qualified class name of the <<kafka-security-Authorizer.adoc#, Authorizer>>, e.g. <<kafka-security-SimpleAclAuthorizer.adoc#, kafka.security.auth.SimpleAclAuthorizer>> (used when `KafkaApis` is requested to <<kafka-server-KafkaApis.adoc#authorize, authorize a principal for an operation on a resource>>)
