...
NOTE: `doTruncate` is used when `AbstractFetcherThread` is requested to <<truncateToHighWatermark, truncateToHighWatermark>> and <<maybeTruncateToEpochEndOffsets, maybeTruncateToEpochEndOffsets>>.
