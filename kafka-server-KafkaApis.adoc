...
`handleTopicMetadataRequest` requests the <<metadataCache, MetadataCache>> for <<kafka-MetadataCache.adoc#getAllTopics, getAllTopics>> or its subset (per <<kafka-common-MetadataRequest.adoc#topics, topics>> attribute of the `MetadataRequest`).
...
`getOrCreateInternalTopic` requests the <<metadataCache, MetadataCache>> for <<kafka-MetadataCache.adoc#getTopicMetadata, getTopicMetadata>> for the input `topic` (and the `ListenerName`).
