...
a| [[logFlushIntervalMessages]]

| logFlushIntervalMs
a| [[logFlushIntervalMs]]
...
Used exclusively when `KafkaServer` is requested for the <<kafka-server-KafkaServer.adoc#copyKafkaConfigToLog, default log properties>> (and set the value of <<kafka-log-LogConfig.adoc#SegmentBytesProp, LogConfig.SegmentBytesProp>>)
