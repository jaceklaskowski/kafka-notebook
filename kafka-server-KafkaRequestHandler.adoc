...
`KafkaRequestHandler` is <<creating-instance, created>> exclusively when `KafkaRequestHandlerPool` is link:kafka-server-KafkaRequestHandlerPool.adoc#runnables[created] (and starts the internal `runnables` threads).
...
NOTE: `run` is used when `KafkaRequestHandlerPool` is link:kafka-server-KafkaRequestHandlerPool.adoc#creating-instance[created].
