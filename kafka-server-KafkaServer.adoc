...
`KafkaServer` can be configured to use a <<kafka-Authorizer.adoc#, Authorizer>> based on <<kafka-properties.adoc#authorizer.class.name, authorizer.class.name>> configuration property (default: `(empty)`).
